...
- **全文搜索** — SQLite **FTS5 trigram 索引**：子串匹配、大小写折叠、CJK 同速；全文只存磁盘、不占内存，内存与粘贴文本大小解耦。图片条目可按备注与来源应用检索
...
- **图片全能** — 支持 PNG/JPEG/GIF/WebP/BMP/TIFF 六种格式捕获，两阶段捕获（先轻量缩略图入列、后异步全量 PNG 编码），图片按需加载
...
| **内存克制** | 全文移出内存 + 4MB LRU page cache（禁用 mmap，FTS 索引不入驻内存）+ 8MB 缩略图缓存（NSCache，countLimit 120）+ 动态阈值裁剪 | 内存与粘贴文本大小解耦，随条目数亚线性增长 |
...
以下均为 **2026-08-19 在 `0.99.20-dev` 构建上实机采集**（macOS 26.6.2 / Apple Silicon，固定随机种子 `42`）。完整流程见 `STRESS=1 ./build.sh`（编译压力测试面板）+ `bash stress_test.sh`（生成数据并采集指标）。

> ⚠️ **这些数字属于 0.99.20-dev，仅供参考趋势而非当前基线。** 其中的「空闲态 14.8 MB 物理内存」尤其不可复现：0.105.x 长会话实测 `vmmap -summary` 的 `Writable regions` 为 60–75 MB `written`（相当部分是已被系统压缩换出的页，不等于真实占用）。**判定内存只认 `vmmap -summary` 的 `Writable regions` 行**；当前基线与构成见 `CLAUDE.md`「内存测量」节。
...
|---|---|---|---|
| **空闲态 · 物理内存 / RSS**（菜单栏，弹窗关闭） | — | — | 历史读数 14.8 / 78.8 MB 已不可复现，不作当前基线；请用 `vmmap -summary` 的 `Writable regions` 判定 |
| **空闲 CPU** | 0.0% | 0.0% | 5 秒 top 采样，changeCount 轻量检测 + 500ms 低频轮询 |
| **冷启动** | ~640 ms | ~644 ms | 从 `open` 到进程出现；条目按需加载 |

两个观察：

- **空闲态内存与条目数解耦**：全文与 FTS 索引只存在 SQLite，内存历史只持元数据与缩略图缓存，条目数增长不带来等量内存增长。0.99.20-dev 曾在 3,000 与 10,000 条下同测 14.8 MB，但该数值在 0.105.x 已不可复现——当前构成见 `CLAUDE.md`「内存测量」节。
...
| **缩略图** | ~0.1 ms | 等比 fit 进行显示框（≤920×200px）CatmullRom 生成 + 编码 |
